...
async function runClient(
  ...
  for (let i = 0; i < transactionCount; i++) {
    try {
      ...
      if ((i + 1) % 1000 === 0) {
        console.log(
          `Client ${clientId}: Completed ${i + 1}/${transactionCount} transactions`,
        );
      }
    } catch (error) {
      failed++;
...
async function runScenario(
  ...
  for (let i = 0; i < numClients; i++) {
    ...
    clientPromises.push(
      runClient(i + 1, userId, transactionsPerClient, amount),
    );
  }
  ...
  console.log("\nFinal balances:");
  const sortedBalances = Object.entries(finalBalances).sort(([a], [b]) =>
    a.localeCompare(b),
  );
...
async function runFunctionalTest(): Promise<void> {
  ...
  console.log("-".repeat(80));
  console.log(
    "Testing basic transactions with positive and negative amounts\n",
  );
  ...
  try {
    ...
    console.log(
      `  - Transaction ${tx1.transaction_ID}: alice +100 -> balance: ${tx1.balance}`,
    );

    const tx2 = await makeTransaction("bob", 50);
    console.log(
      `  - Transaction ${tx2.transaction_ID}: bob +50 -> balance: ${tx2.balance}`,
    );

    const tx3 = await makeTransaction("alice", 25);
    console.log(
      `  - Transaction ${tx3.transaction_ID}: alice +25 -> balance: ${tx3.balance}`,
    );

    console.log("\nTest 2: Creating transactions with negative amounts...");
    const tx4 = await makeTransaction("alice", -30);
    console.log(
      `  - Transaction ${tx4.transaction_ID}: alice -30 -> balance: ${tx4.balance}`,
    );

    const tx5 = await makeTransaction("bob", -10);
    console.log(
      `  - Transaction ${tx5.transaction_ID}: bob -10 -> balance: ${tx5.balance}`,
    );

    const tx6 = await makeTransaction("charlie", 75);
    console.log(
      `  - Transaction ${tx6.transaction_ID}: charlie +75 -> balance: ${tx6.balance}`,
    );

    const tx7 = await makeTransaction("charlie", -25);
    console.log(
      `  - Transaction ${tx7.transaction_ID}: charlie -25 -> balance: ${tx7.balance}`,
    );
    ...
    console.log(
      `  Alice: balance = ${aliceInfo.balance}, transactions = ${aliceInfo.logs.length}`,
    );
    const expectedAliceBalance = 100 + 25 - 30;
    if (aliceInfo.balance === expectedAliceBalance) {
      console.log(
        `  - Alice balance is correct (expected ${expectedAliceBalance})`,
      );
    } else {
      console.error(
        `  !!! Alice balance is INCORRECT (expected ${expectedAliceBalance}, got ${aliceInfo.balance})`,
      );
    }

    const bobInfo = await getUserInfo("bob");
    console.log(
      `  Bob: balance = ${bobInfo.balance}, transactions = ${bobInfo.logs.length}`,
    );
    const expectedBobBalance = 50 - 10;
    if (bobInfo.balance === expectedBobBalance) {
      console.log(
        `  - Bob balance is correct (expected ${expectedBobBalance})`,
      );
    } else {
      console.error(
        `  !!! Bob balance is INCORRECT (expected ${expectedBobBalance}, got ${bobInfo.balance})`,
      );
    }

    const charlieInfo = await getUserInfo("charlie");
    console.log(
      `  Charlie: balance = ${charlieInfo.balance}, transactions = ${charlieInfo.logs.length}`,
    );
    const expectedCharlieBalance = 75 - 25;
    if (charlieInfo.balance === expectedCharlieBalance) {
      console.log(
        `  - Charlie balance is correct (expected ${expectedCharlieBalance})`,
      );
    } else {
      console.error(
        `  !!! Charlie balance is INCORRECT (expected ${expectedCharlieBalance}, got ${charlieInfo.balance})`,
      );
    }
    ...
    for (const [userId, balance] of Object.entries(allBalances).sort(
    ...
    for (const log of aliceInfo.logs) {
      console.log(
        `    ${log.timestamp}: ${log.msg.amount > 0 ? "+" : ""}${log.msg.amount}`,
      );
    }
    ...
    if (allCorrect) {
      console.log(
        "FUNCTIONAL TEST PASSED: All balances and logs are correct!",
      );
    } else {
      console.log(
        "FUNCTIONAL TEST FAILED: Some balances or logs are incorrect!",
      );
    }
    console.log("-".repeat(80) + "\n");
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    const expectedAccounts = Array.from(
      { length: 10 },
      (_, i) => `user-${i + 1}`,
    );
    ...
    for (const userId of expectedAccounts) {
      ...
      if (balance !== expected) {
        console.error(
          `!!! Scenario 1 FAILED: ${userId} has balance ${balance}, expected ${expected}`,
        );
    ...
    if (sharedBalance === expectedShared) {
      console.log(
        `- Scenario 2 PASSED: Shared account has correct balance (${sharedBalance})\n`,
      );
    } else {
      console.error(
        `!!! Scenario 2 FAILED: Shared account has balance ${sharedBalance}, expected ${expectedShared}\n`,
      );
    }
